...
- `scikit-learn` (sklearn) — modeling and preprocessing building blocks used in `project.py`:
  - `sklearn.compose.ColumnTransformer` — applies different preprocessing to subsets of columns (here: one-hot encoding of the `Region` column while passing through numeric features unchanged). Keeps preprocessing logic tidy and compatible with `Pipeline`.
  - `sklearn.pipeline.Pipeline` — chains preprocessing + estimator. Ensures the same transforms applied at training and prediction time and provides a single `.fit()`/.predict()` interface.
  ...
  - `sklearn.ensemble.HistGradientBoostingRegressor` — the chosen estimator. See section below for motivation.
...
- Histogram-based algorithm: HGBR bins continuous features into histograms before splitting. This often gives large speedups on medium-to-large tabular datasets versus classical gradient boosting implementations, while keeping similar accuracy.
- Performance and memory: HGBR is optimized in scikit-learn and tends to be faster and more memory-efficient than the older GradientBoosting implementation for many datasets.
...
- If your data contains many missing values and you'd like native handling, check the specific sklearn version's behavior; some tree implementations treat NaN specially while others require imputation.
...
3. Training: a time-based per-region holdout is used (last `cfg.test_quarters` per region are test rows). The pipeline is fit on the training portion.
